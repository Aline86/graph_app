...
```bash
git clone https://github.com/Aline86/graph_app.git
cd graph_app
npm install
npm run dev
```
...
- Création, déplacement, renommage et suppression de nœuds
- Tracé d'arêtes orientées. Pour avoir une arête bidirectionnelle, il s'agit de créer l'arête aller et l'arête retour.
- Visualisation pas à pas de l'algorithme BFS. Le nœud de départ est calculé en fonction de son nombre d'arêtes sortantes.
- Le graphe s'enregistre dans le navigateur et est rechargé au démarrage.
...
Registre statique qui mappe des identifiants d'actions à des handlers. Les ids sont des identifiants de **catégorie** (`node_events`, `arrow_events`) associés à l'identifiant de l'instance de graphe auquel le bouton et son action associée doivent se rattacher.
 
Concrètement, ajouter une action sur les nœuds ne demande aucune modification du code existant : un `FigureRegistry.register()` avec le bon `actions_id` suffit, le bouton est créé automatiquement sur chaque nœud.
 
---
 
## MVVM plutôt que MVC
 
Il est difficile de scinder les évènements des éléments DOM avec une structure MVC, on arrive rapidement à un modèle hybride et difficilement maintenable. MVVM est plus adapté si on veut rajouter des fonctionnalités liées aux évènements sans mélanger la logique associée aux évènements DOM et l'affichage des éléments.
...
6. Déclarer le module dans `AlgorithmModule` (voir limites connues)
```
...
- J'ai choisi d'utiliser un bus d'évènements plutôt qu'un store pour trigger et dispatcher les évènements DOM car un store gère l'état interne de l'app. Le store mute les valeurs alors que le bus génère juste un évènement de réaction : le store aurait mélangé les responsabilités, et le code également par conséquent.
- J'ai choisi un découplage modulaire pour ne pas avoir à toucher à la logique de création et d'édition de graphe lorsque je rajoute des algorithmes. L'objectif, c'est de pouvoir rajouter plein de modules sans toucher à la logique centralisée.
...
- **Écrire les tests** (`BFSLogic`, `Graph.create_hash_map` et le calcul du nœud de départ sont de la logique pure, testables sans DOM).